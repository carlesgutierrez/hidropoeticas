...
public class hidropoeticas_FFTTheremin extends PApplet {
...
public float thresholdMinimInteractionFFT = 0.0003f;
...
public int cBars = color(0xFF00FFFF);//#00ffff
...
public void createCUSTOMGUI(int _x, int _y) {
  ...
  cp5.addColorWheel("cBars", 450, 10, 100 ).setRGB(color(0xFF00FFFF));
  ...
  cp5.addToggle("bOSCActive")
    .setPosition(_x+width*.5f, _y+40)
    .setSize(50, 10)
    ;

  // add a vertical slider
  cp5.addSlider("typeOSCMode")
    .setPosition(_x+width*.55f, _y+40)
    .setSize(100, 10)
    .setRange(1, 2)
    .setNumberOfTickMarks(2)
    ;
  ...
  cp5.addToggle("bCircleDrawer")
    .setPosition(_x+width*0.8f, _y+0)
    .setSize(50, 50)
    ;
...
public void setup() {
  ...
  /* size commented out by preprocessor */; //640, 360
...
public void draw() {
  ...
  //draw Calcs
  float auxFreq = PApplet.parseInt(map(maxFId, 0, bandsThreshold, 0, 1920));//Map into FULLHD width
  float auxFreqAmplitude = PApplet.parseInt(map(getMaxValueFFT(maxFId), 0, 0.2f, 0, height));
...
public int findIdInteraction(int _maxFId) {
  return PApplet.parseInt(map(_maxFId, 0, bandsThreshold, 0, numVideoCtrl));
}

//----------------------------------------------------
public void updatePctInteraction(int _maxFId) {

  float auxFreq = PApplet.parseInt(map(_maxFId, 0, bandsThreshold, 0, 1920));
  ...
  if(idVideoSelected == last_idVideoSelected){
    pctLerp = lerp(pctLerp, pctAux, 0.05f);//0.01
  }else {
    pctLerp = pctAux;
...
public void sendOSCAbletonFreqData(int _idBandMaxFr) {
  ...
  if (_idBandMaxFr >0 && _idBandMaxFr < bands) {
    ...
    float auxFreqAmplitude = map(getMaxValueFFT(_idBandMaxFr), 0.0f, 0.2f, 0.0f, 1.0f);//Map into [0, 100]
...
public void sendOSCArenaVideoData(int _idBandMaxFr) {
  ...
  if (_idBandMaxFr >0 && _idBandMaxFr < bands) {
    ...
    if (_idBandMaxFr >0 && _idBandMaxFr < bands) {
      ...
      float auxFreqAmplitude = map(getMaxValueFFT(_idBandMaxFr), 0, 0.2f, 0, 1);// until 0.5 it's ok
...
public void drawCustomFFTMode(int _mode, int _maxFreqIdBand) {
  ...
  if (_maxFreqIdBand >0 && _maxFreqIdBand < bands && bCircleDrawer) {
    ...
    int auxPosX = PApplet.parseInt(gapX*_maxFreqIdBand + _maxFreqIdBand*barWidth);
    pctCirclePosXLerp = lerp(pctCirclePosXLerp, auxPosX, 0.05f);
    
    int auxPosY = PApplet.parseInt(getMaxValueFFT(_maxFreqIdBand)*height*scaleRectH);
...
pixelDensity(displayDensity()); }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "hidropoeticas_FFTTheremin" };
    if (passedArgs != null) {
